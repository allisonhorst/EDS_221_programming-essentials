...
In that case, we may want to apply our function over a range of growth rates (from 0.05 to 0.40), for the time sequence we used here (0 to 35). 

```{r}

# Create a sequence of growth rate values: 
r_seq <- seq(from = 0.05, to = 0.40, by = 0.05)
...
Now some wrangling so we can graph it: 
```{r}
...
out_df_long <- out_df %>% 
  pivot_longer(cols = 1:8, names_to = "growth_rate", values_to = "population")

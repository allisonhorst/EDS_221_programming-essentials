...
- In a single R Markdown document:

  - Read in both files (ice cover duration and meteorological data)
  
  - Find the mean ice cover duration in Madison Lake Area by year. Understanding check: what are you finding the mean of here? Why are there multiple observations for each year?)
  ...
  - Explore thaw dates since 1925. Create a subset of the ice cover duration data since 1925, then convert the `ice_off` column to a `Date`. Use `lubridate::yday()` to pull the numeric *day of the year* from that Date. Create an exploratory plot of the ice off day (numeric day-of-the-year) from 1925 - present. Add a sentence or two below this exploratory graph describing the overall trend(s) you observe. 
